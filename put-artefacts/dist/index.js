...
async function uploadS3object({
  ...
  const src = (0, import_path2.resolve)(workspacePath, path);
  const isDirectory = (await import_fs3.promises.lstat(src)).isDirectory();
  const dest = getS3url({
    component,
    sha,
    usePlatformPrefix,
  ...
  const cmd = [
    ...
    S3_REGION,
    `"${src}"`,
    isDirectory ? `"${dest}"` : `"${dest}/${(0, import_path2.basename)(path)}"`
...
async function zipAll(path, component) {
  ...
  const cwd = (0, import_path3.resolve)(getWorkspacePath(), path);
