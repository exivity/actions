...
async function execGit(command, silent = true) {
  return (await (0, import_exec.getExecOutput)(command, void 0, { silent })).stdout;
}
...
${JSON.stringify(error, void 0, 2)}`);
  ...
  async resolveChannelToUserId(value) {
    ...
    if (value.startsWith("#")) {
      ...
      const channelMatch = channels.find((item) => item.name === value.substring(1));
...
async function run() {
  ...
  const prBlock = pr ? [
    {
      type: "mrkdwn",
      text: `\u{1F64F} <https://github.com/exivity/${component}/pull/${pr.number}|#${pr.number}>`
    }
  ] : [];
  const shaBlock = {
    type: "mrkdwn",
    text: `\u{1F511} <https://github.com/exivity/${component}/commit/${sha}|${sha.substring(0, 7)}>`
  };
