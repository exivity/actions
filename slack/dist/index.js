...
${JSON.stringify(error, void 0, 2)}`
  ...
  async resolveChannelId(value) {
    ...
    if (value.startsWith("#")) {
      ...
      return (channelMatch == null ? void 0 : channelMatch.id) || null;
    }
    return value;
  }
  async findUserFuzzy(values) {
    ...
    const userMatch = users.find(
      (item) => values.includes(item.name) || values.includes(item.real_name) || values.includes(item.profile.display_name) || values.includes(item.profile.display_name_normalized) || values.includes(item.profile.email) || values.includes(item.profile.real_name) || values.includes(item.profile.real_name_normalized)
    );
    return userMatch;
  }
};
...
// slack/src/index.ts
var validStatuses = ["", "success", "failure", "cancelled"];
...
async function run() {
  ...
  if (!channel) {
    (0, import_core6.warning)(
      `Please set the "channel" input or make sure this action can match the author of the commit triggering this workflow to a Slack user (tried with "${authorName}", "${authorEmail}" and "${import_github2.context.action}").`
    );
    (0, import_console2.info)(
      `This action will try to match the author of the commit using the Slack user attributes "name", "display_name", "real_name" or "email".`
    );
    if (fallbackChannel) {
  ...
  const prBlock = pr ? [
    {
      type: "mrkdwn",
      text: `\u{1F64F} <https://github.com/${fqn}/pull/${pr.number}|#${pr.number}>`
    }
  ] : [];
  const shaBlock = {
    type: "mrkdwn",
    text: `\u{1F511} <https://github.com/${fqn}/commit/${sha}|${sha.substring(0, 7)}>`
  };
  const commitMessageBlock = {
    type: "mrkdwn",
    text: `\u{1F524} ${commitMessage}`
  };
  const actorBlock = {
    type: "mrkdwn",
